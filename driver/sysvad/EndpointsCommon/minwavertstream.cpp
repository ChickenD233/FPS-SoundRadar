...
--*/
{
    ...
    m_bLoopbackProducer = FALSE;
    m_bLoopbackConsumer = FALSE;
    m_pAudioModules = NULL;
    m_AudioModuleCount = 0;
    ...
    //
    // SoundRadar VAD: classify this stream against the loopback ring. Only
    // the speaker endpoint participates: its host render pin feeds the ring
    // and its loopback pin drains it. Other endpoints (HDMI, SPDIF, mics,
    // offload) keep their existing behavior.
    //
    if (m_pMiniport->m_DeviceType == eSpeakerDevice)
    {
        if (!m_bCapture && m_pMiniport->IsSystemRenderPin(m_ulPin))
        {
            m_bLoopbackProducer = TRUE;
            LoopbackRingSetRenderFormat(m_pWfExt);
        }
        else if (m_bCapture && m_pMiniport->IsLoopbackPin(m_ulPin))
        {
            m_bLoopbackConsumer = TRUE;
            LoopbackRingCheckCaptureFormat(m_pWfExt);
        }
    }
...
)
{
    ...
    else
    {
        ...
        if (!g_DoNotCreateDataFiles || m_bLoopbackProducer)
        {
            // Read from buffer: save to a file (if enabled) and/or feed
            // the SoundRadar loopback ring.
...
This function writes the audio buffer: loopback capture streams are filled
from the render->loopback ring, all other capture streams use the sine wave
generator.
Arguments:
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        if (m_bLoopbackConsumer)
        {
            // SoundRadar VAD: fill the loopback capture buffer with the
            // bytes that were rendered to the speaker host pin.
            LoopbackRingPopBytes(m_pDmaBuffer + bufferOffset, runWrite);
        }
        else
        {
...
--*/
{
    ...
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        if (m_bLoopbackProducer)
        {
            // SoundRadar VAD: tee the consumed render bytes into the
            // loopback ring for the loopback capture pin.
            LoopbackRingPushBytes(m_pDmaBuffer + bufferOffset, runWrite);
        }
