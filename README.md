...
- 每个可调项下面都有一行小字，写明往哪边拖、会发生什么。灵敏度滑块决定箭头亮度和多久变红；能否检测到声音由"检测门限"决定。颜色刻度相对实测环境噪声底，所以低音量混音也能看清，不会一片漆黑。
